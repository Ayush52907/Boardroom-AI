export const EXECUTIVE_STYLES: Record<string, { initials: string; color: string; name: string }> = {
...
export function renderBriefingHtml(brief: any, companyName?: string): string {
...
export function renderDecisionHtml(discussion: any, companyName?: string): string {
  const companyStr = companyName ? `<div class="company">${escapeHtml(companyName)}</div>` : "";
